...
type FormStatus = 'idle' | 'loading' | 'success' | 'success-demo' | 'error'
...
export function NewsletterForm({
  ...
  // Success state
  if (status === 'success' || status === 'success-demo') {
    return (
      <div className={`flex flex-col gap-1 ${className}`}>
        ...
        {status === 'success-demo' && (
          <p className="text-xs text-gray-500">
            Email logged (configure Mailchimp/Klaviyo for production)
          </p>
        )}
      </div>
    )
